...
print_summary(double refresh_time,struct sysvminfo *sv1,struct sysvminfo *sv2)
{
  ...
  mon_print(2,0, "Real memory:    %6.1lfM free %6.1lfM procs %6.1lfM files %6.1lfM total",
	    vmk->memfree/1024.0,
	    (vmk->memtotal-vmk->memfree-vmk->buffers- vmk->buffers-vmk->cached)/1024.0, 
	    vmk->buffers/1024.0,
	    vmk->memtotal/1024.0);
...
void print_mem(char *str, long long mem)
{
  ...
  } else if (mem < 10000*1024*1024) {
    sprintf(str, "%4.0fM", mem/1024.0/1024.0);
  } else if (mem < 100*1024*1024*1024) {
...
print_topcpu(double refresh_time, topcpu_t *top, int ntop)
{
  ...
  if (!topflag_usersystemtime) {
    ...
    mon_print(y,0,"   PID USER     PRI NICE  SIZE   RES PFLT ST  USERTIME    SYSTIME   CPU%% COMMAND");y++;
	...
	sprintf(username,"%d",top[i].uid);
      ...
      print_mem(vsize, top[i].memsize_1k*1024);
      print_mem(rsize, top[i].ressize_1k*1024);
      mon_print(y+i,0, "%6d %-8s %3d %3d  %s %s %4.0f %c %4d:%02d.%02d %4d:%02d.%02d %5.1f%% %s\n",
		top[i].pid,	   // %6d
		username,          // %-8d
		top[i].pri,	   // %3d
		top[i].nice,    // 3d
